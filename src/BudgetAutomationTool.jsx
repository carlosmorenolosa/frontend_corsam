...
const SummaryCard = ({ icon, title, value, subtitle, colorClass }) => {
  const Icon = icon;
  return (
    <div className={`bg-white rounded-2xl p-6 border ${colorClass.border} shadow-lg shadow-slate-500/5`}>
      <div className="flex items-center mb-3">
        <div className={`w-10 h-10 rounded-full flex items-center justify-center mr-4 ${colorClass.bg}`}>
...
const BudgetAutomationTool = () => {
  ...
  const handleAddItem = () => {
    ...
    const newItem = { id: nextId, description: "", quantity: 1, unit: "ud", currentPrice: 0, targetRate: 50, materialsMargin: 30 };
      ...
      switch(currentStep) {
                ...
                <div className="text-center w-full max-w-2xl mx-auto">
                    <div
                      className={`relative border-2 border-dashed rounded-3xl p-10 transition-all duration-300 ${
                        dragActive ? 'border-blue-600 bg-blue-50 scale-105 shadow-xl' : 
                    ...
                    >
                      <div className="w-16 h-16 mx-auto mb-4 bg-blue-100 rounded-full flex items-center justify-center">
                        <Upload className={`w-8 h-8 transition-colors ${remainingBudgets <= 0 ? 'text-slate-400' : 'text-blue-600'}`} />
                      </div>
                      <h3 className="text-xl font-bold text-slate-800 mb-2">Sube tu presupuesto de obra</h3>
                      ...
                      <button 
                        ...
                        className="px-8 py-3 rounded-lg font-semibold text-white bg-gradient-to-r from-blue-600 to-blue-500 hover:scale-105 transition-all duration-300 transform disabled:from-slate-400 disabled:to-slate-400 disabled:cursor-not-allowed disabled:hover:scale-100 shadow-lg hover:shadow-blue-500/30"
                      >
        ...
        case 1: // Processing step
            return (
                <div className="text-center flex flex-col items-center justify-center h-full">
                    <Loader2 className="w-14 h-14 text-blue-600 mx-auto mb-6 animate-spin" />
                    <h3 className="text-2xl font-bold text-slate-800 mb-2">Analizando con IA...</h3>
            ...
            return (
                extractedData && <div className="w-full">
                    <div className="w-full flex justify-end mb-4">
                        <button onClick={resetProcess} className="bg-slate-100 hover:bg-slate-200 text-slate-700 font-semibold py-2 px-4 rounded-lg transition-colors text-sm">
                            Volver a Inicio
                        </button>
                    </div>
                      ...
                      <div className="flex flex-wrap items-end gap-4">
                        <div>
                          ...
                          <input 
                            ...
                            className="p-2 bg-white border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 transition w-40"
                          />
                        </div>
                        <div>
                    ...
                    <div className="space-y-3 bg-white p-4 rounded-2xl border border-slate-200 max-h-[55vh] overflow-y-auto shadow-inner">
                        ...
                        {extractedData.items.map((item, index) => (
                            <div key={item.id} className="grid grid-cols-12 gap-4 items-center bg-slate-50 p-3 rounded-lg border border-slate-200">
                                <textarea value={item.description} onChange={(e) => handleExtractedDataChange(index, 'description', e.target.value)} className="col-span-12 md:col-span-5 p-2 bg-white border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 transition" rows="3"></textarea>
                                <input type="number" value={item.quantity} onChange={(e) => handleExtractedDataChange(index, 'quantity', e.target.value)} className="col-span-6 md:col-span-1 p-2 bg-white border border-slate-300 rounded-lg text-center focus:ring-2 focus:ring-blue-500 transition" />
                                <input type="text" value={item.unit} onChange={(e) => handleExtractedDataChange(index, 'unit', e.target.value)} className="col-span-6 md:col-span-1 p-2 bg-white border border-slate-300 rounded-lg text-center focus:ring-2 focus:ring-blue-500 transition" />
                                <input type="number" value={item.targetRate} onChange={(e) => handleExtractedDataChange(index, 'targetRate', e.target.value)} className="col-span-6 md:col-span-2 p-2 bg-white border border-slate-300 rounded-lg text-center focus:ring-2 focus:ring-blue-500 transition" />
                                <input type="number" value={item.materialsMargin} onChange={(e) => handleExtractedDataChange(index, 'materialsMargin', e.target.value)} className="col-span-6 md:col-span-2 p-2 bg-white border border-slate-300 rounded-lg text-center focus:ring-2 focus:ring-blue-500 transition" />
                     ...
                     <div className="text-center mt-4">
                      <button onClick={handleAddItem} className="bg-slate-200 hover:bg-slate-300 text-slate-700 text-sm font-medium px-4 py-2 rounded-lg transition-colors">
                        + Añadir Partida
                      </button>
                    </div>
                    ...
                    <div className="mt-12">
                         <div className="bg-amber-50 border border-amber-200 rounded-2xl p-6">
                            ...
                            <div className="bg-white rounded-lg p-4 border border-amber-200 shadow-inner">
                                {isAuditing ? (
                    ...
                    <div className="text-center mt-10">
                        <button onClick={handleConfirmAndOptimize} className="bg-gradient-to-r from-green-600 to-emerald-500 text-white px-10 py-4 rounded-lg font-semibold flex items-center mx-auto hover:scale-105 transition-transform shadow-lg hover:shadow-green-500/30 text-base">
        ...
        case 3:
             return (
                <div className="text-center flex flex-col items-center justify-center h-full">
                     <Loader2 className="w-14 h-14 text-blue-600 mx-auto mb-6 animate-spin" />
                     <h3 className="text-2xl font-bold text-slate-800 mb-2">Optimizando Presupuesto...</h3>
            ...
            return (
               optimizedBudget && <div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-5 mb-8">
                      <SummaryCard icon={Clock} title="Horas totales" value={optimizedBudget.totalHours.toFixed(2)} colorClass={{bg:'bg-slate-100', border:'border-slate-200', text:'text-slate-600'}}/>
                      <SummaryCard icon={DollarSign} title="Beneficio estimado" value={`${optimizedBudget.totalProfit.toFixed(2)} €`} colorClass={{bg:'bg-green-100', border:'border-green-200', text:'text-green-700'}}/>
                      <SummaryCard icon={BarChart3} title="Rentabilidad" value={`${optimizedBudget.profitPerHour.toFixed(2)} €/h`} colorClass={{bg:'bg-blue-100', border:'border-blue-200', text:'text-blue-700'}}/>
                    </div>
                      ...
                      <div className="overflow-x-auto">
                        <table className="w-full text-sm text-left">
                          <thead className="bg-slate-50">
                            <tr>
                              <th className="px-4 py-3 font-semibold text-slate-600 w-1/12">Código</th>
                              <th className="px-4 py-3 font-semibold text-slate-600 w-4/12">Descripción</th>
                              <th className="px-4 py-3 text-center font-semibold text-slate-600 w-1/12">Cant.</th>
                              <th className="px-4 py-3 text-right font-semibold text-slate-600 w-2/12">Precio IA (total)</th>
                              <th className="px-4 py-3 text-right font-semibold text-slate-600 w-2/12">Beneficio € (total)</th>
                              <th className="px-4 py-3 text-center font-semibold text-slate-600 w-1/12">Detalles</th>
                            </tr>
                          </thead>
                          <tbody>
                            {optimizedBudget.items.map((item, index) => (
                              <React.Fragment key={index}>
                                <tr className="border-t border-slate-200 hover:bg-slate-50 transition-colors">
                                  <td className="px-4 py-3 text-slate-700 font-medium"><input type="text" value={item.code || '---'} onChange={(e) => handleOptimizedDataChange(index, 'code', e.target.value)} className="w-full bg-transparent p-1 rounded-md focus:bg-white focus:ring-1 focus:ring-blue-500"/></td>
                                  <td className="px-4 py-3 text-slate-700 font-medium">{item.description}</td>
                                  <td className="px-4 py-3 text-center text-slate-600"><input type="number" value={item.quantity} onChange={(e) => handleOptimizedDataChange(index, 'quantity', e.target.value)} className="w-20 bg-transparent p-1 rounded-md focus:bg-white focus:ring-1 focus:ring-blue-500 text-center"/> {item.unit}</td>
                                  ...
                                  <td className={`px-4 py-3 text-right font-semibold ${ tot(item,'profitUnit') < 0 ? 'text-red-500' : 'text-slate-800' }`}>
                                    {tot(item,'profitUnit').toFixed(2)} €
                                  </td>
                                  <td className="px-4 py-3 text-center">
                                    <button onClick={() => setOpenRow(openRow === index ? null : index)} className="p-2 text-slate-500 hover:bg-slate-200 rounded-full transition-colors">
                                      <ChevronDown className={`w-5 h-5 transition-transform ${openRow === index ? 'rotate-180' : ''}`} />
                                    </button>
                                  </td>
                                </tr>
                                {openRow === index && (
                                  <tr className="bg-slate-50">
                                    <td colSpan={6} className="p-4">
                                      <div className="grid grid-cols-2 gap-x-8 gap-y-4 bg-white p-4 rounded-lg border border-slate-200">
                                        <div className="text-sm"><span className="font-semibold text-slate-600">Horas est. totales:</span> <input type="number" value={tot(item,'hoursUnit').toFixed(2)} onChange={(e) => handleOptimizedDataChange(index, 'hoursUnitTotal', e.target.value)} className="w-24 bg-slate-100 p-1 rounded-md focus:bg-white focus:ring-1 focus:ring-blue-500 text-right"/></div>
                                        <div className="text-sm"><span className="font-semibold text-slate-600">Material € (total):</span> <input type="number" value={tot(item,'materialUnit').toFixed(2)} onChange={(e) => handleOptimizedDataChange(index, 'materialUnitTotal', e.target.value)} className="w-24 bg-slate-100 p-1 rounded-md focus:bg-white focus:ring-1 focus:ring-blue-500 text-right"/></div>
                                        <div className="text-sm"><span className="font-semibold text-slate-600">Subcontrata € (total):</span> <input type="number" value={tot(item,'contrataUnit').toFixed(2)} onChange={(e) => handleOptimizedDataChange(index, 'contrataUnitTotal', e.target.value)} className="w-24 bg-slate-100 p-1 rounded-md focus:bg-white focus:ring-1 focus:ring-blue-500 text-right"/></div>
                                        <div className="text-sm"><span className="font-semibold text-slate-600">Mano de Obra € (total):</span> <input type="number" value={tot(item,'manoObraUnit').toFixed(2)} onChange={(e) => handleOptimizedDataChange(index, 'manoObraUnitTotal', e.target.value)} className="w-24 bg-slate-100 p-1 rounded-md focus:bg-white focus:ring-1 focus:ring-blue-500 text-right"/></div>
                    ...
                    <div className="flex justify-center space-x-4">
                        <button onClick={() => handleGenerateBC3()} className="bg-gradient-to-r from-blue-600 to-blue-500 text-white px-8 py-3 rounded-lg font-semibold hover:scale-105 transition-all duration-300 flex items-center shadow-lg hover:shadow-blue-500/30"><Package className="w-5 h-5 mr-2" /> Generar Presupuesto en BC3</button>
                    </div>
               </div>
            );
        case 5: // Review BC3 step
            return (
                processing ? (
                    <div className="text-center flex flex-col items-center justify-center h-full">
                        <Loader2 className="w-14 h-14 text-blue-600 mx-auto mb-6 animate-spin" />
                        <h3 className="text-2xl font-bold text-slate-800 mb-2">Generando Presupuesto BC3...</h3>
                    ...
                    generatedBc3Content && <div>
                        ...
                        <div className="mb-6">
                            <textarea
                                className="w-full h-96 p-4 border border-slate-300 rounded-lg font-mono text-xs bg-slate-50 focus:ring-2 focus:ring-blue-500 transition"
                                value={generatedBc3Content}
                                onChange={(e) => setGeneratedBc3Content(e.target.value)}
                            />
                        </div>
                        <div className="text-center mt-8">
                            <button onClick={handleDownloadFinalBC3} className="bg-gradient-to-r from-green-600 to-emerald-500 text-white px-10 py-4 rounded-lg font-semibold flex items-center mx-auto hover:scale-105 transition-transform shadow-lg hover:shadow-green-500/30 text-base">
  ...
  return (
    <>
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="bg-white/70 backdrop-blur-xl border border-slate-200/80 shadow-lg shadow-blue-500/5 rounded-2xl p-6 mb-8">
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6">
              <div className="flex items-center">
                  ...
                  <div>
                      ...
                      <p className="text-slate-600">Sube un archivo para analizar, optimizar y generar un presupuesto en formato BC3.</p>
                  </div>
              </div>
              <div className="bg-slate-50 rounded-xl p-4 border border-slate-200 w-full sm:w-auto flex-shrink-0">
                  <div className="text-center">
                    ...
                    <div className="w-full bg-slate-200 rounded-full h-2.5">
                      <div 
                        className={`h-2.5 rounded-full transition-all duration-500 ${progressPercentage > 80 ? 'bg-red-500' : progressPercentage > 60 ? 'bg-orange-500' : 'bg-blue-600'}`} 
        ...
        {remainingBudgets <= 5 && remainingBudgets > 0 && (
            <div className="bg-orange-100 border border-orange-200 rounded-xl p-4 mb-8 flex items-center text-orange-800">
        ...
        {remainingBudgets <= 0 && (
            <div className="bg-red-100 border border-red-200 rounded-xl p-4 mb-8 flex items-center text-red-800">
              <AlertCircle className="w-6 h-6 mr-3 flex-shrink-0" />
              <div>
        ...
        <div className="mb-12">
            <div className="relative max-w-4xl mx-auto">
                ...
                <div
                    className="absolute top-1/2 -translate-y-1/2 left-0 h-1 bg-blue-600 transition-all duration-500"
                  ...
                  {steps.map((step, index) => (
                      <div key={index} className="flex flex-col items-center text-center z-10">
                          <div className={`w-12 h-12 rounded-full flex items-center justify-center transition-all duration-300 border-4 relative ${
                            index < currentStep ? 'bg-green-500 border-green-200' : 
                            index === currentStep ? 'bg-blue-600 border-blue-200 scale-110 shadow-lg shadow-blue-500/30' : 
                          ...
                          <p className={`mt-3 font-semibold text-xs transition-colors ${
                            index < currentStep ? 'text-slate-700' : 
        ...
        {/* Main Step Container */}
        <div className="bg-white/70 backdrop-blur-xl border border-slate-200/80 shadow-2xl shadow-blue-500/5 rounded-3xl p-6 sm:p-10 min-h-[450px] flex items-center justify-center">
